...
create table recommend.tag_relation_cate_user_preference_level_3 as select user_proxy_key,tag_id,user_tag_weight from recommend.dw_cp_user_preference_long_term  a left join recommend.sync_level_3_1 b on a.tag_id=b.level_3_id and a.ds>'2017-09-23' where b.level_3_id is not null;
select '限制用户最大浏览20 和最小数值0.005 后，12447811/2451193（少的用户是因为权重都小于0.005？）';
CREATE TABLE recommend.tag_relation_cate_user_preference_level_3_simple AS select tag_id,user_proxy_key from (SELECT tag_id,user_proxy_key,user_tag_weight,count(*) over (partition by user_proxy_key) tag_sum, row_number () over (partition by user_proxy_key ORDER BY user_tag_weight DESC) rank FROM recommend.tag_relation_cate_user_preference_level_3) t where tag_sum>5 and rank<20 and user_tag_weight>0.005;

select '计算标签出现次数';
CREATE TABLE recommend.tag_relation_cate_count_left AS SELECT tag_id,count(DISTINCT user_proxy_key) user_num,row_number () over (ORDER BY count(DISTINCT user_proxy_key) DESC) rank FROM recommend.tag_relation_cate_user_preference_level_3_simple GROUP BY tag_id where user_num > 10;
...
select '计算标签出现次数';
CREATE TABLE recommend.tag_relation_brand_count_left AS SELECT tag_id,count(DISTINCT user_proxy_key) user_num,row_number () over (ORDER BY count(DISTINCT user_proxy_key) DESC) rank FROM recommend.tag_relation_brand_user_preference_simple GROUP BY tag_id where user_num > 10;
...
select '三级品类协同过滤数据源计算';
  ...
  select '计算共现矩阵';
  CREATE TABLE recommend.tag_relation_catebrand_count_cross AS SELECT t.tag_id_1,t.tag_id_2,t.num FROM (SELECT t1.tag_id AS tag_id_1,t2.tag_id AS tag_id_2,count(DISTINCT t2.user_proxy_key) AS num FROM recommend.tag_relation_cate_user_preference_level_3_simple t1 CROSS JOIN recommend.tag_relation_brand_user_preference_simple t2 ON t1.user_proxy_key=t2.user_proxy_key WHERE t1.tag_id<> t2.tag_id GROUP BY t1.tag_id,t2.tag_id) t;

  select '标签之间相似度';
  CREATE TABLE recommend.tag_relation_catebrand_collaborative AS SELECT t1.tag_id_1 AS tag_id_1,t2.user_num_1 AS user_num_1,t1.tag_id_2 AS tag_id_2,t3.user_num_2 AS user_num_2,t1.num AS num,(t1.num/sqrt(t2.user_num_1*t3.user_num_2)) AS power,row_number () over (ORDER BY (t1.num/sqrt(t2.user_num_1*t3.user_num_2)) DESC) rank FROM recommend.tag_relation_catebrand_count_cross t1 LEFT JOIN (SELECT tag_id,user_num AS user_num_1 FROM recommend.tag_relation_brand_count_left) t2 ON t1.tag_id_1=t2.tag_id LEFT JOIN (SELECT tag_id,user_num AS user_num_2 FROM recommend.tag_relation_cate_count_right) t3 ON t1.tag_id_2=t3.tag_id GROUP BY t1.tag_id_1,t2.user_num_1,t1.tag_id_2,t3.user_num_2,t1.num,(t1.num/sqrt(t2.user_num_1*t3.user_num_2));

  CREATE TABLE recommend.tag_relation_catebrand_collaborative_top20 AS SELECT tag_id_1,tag_id_2,power,rank FROM (SELECT tag_id_1,tag_id_2,power,row_number () over (PARTITION BY tag_id_1 ORDER BY power DESC) rank FROM recommend.tag_relation_catebrand_collaborative) t1 WHERE t1.rank<20;

  create table recommend.tag_relation_catebrand_collaborative_top20_name as select d.*,c.tag_name from recommend.dw_cp_dic_tag_info c right join  (select a.*,b.tag_name as tag_relation_brand from recommend.tag_relation_catebrand_collaborative_top20 a left join recommend.dw_cp_dic_tag_info b on a.tag_id_1=b.tag_id and b.ds='2017-10-23') d on c.tag_id=d.tag_id_2 and c.ds='2017-10-23';
"
