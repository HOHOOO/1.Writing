...
drop table recommend.tag_relation_user_preference_brand;
drop table recommend.tag_relation_user_preference_brand_simple;
...
create table recommend.tag_relation_user_preference_brand as select user_proxy_key,tag_id,user_tag_weight from recommend.tag_relation_user_preference_long_term  where  channel='brand';
select "限制用户最大浏览20 和最小数值0.005 后，12447811/2451193（少的用户是因为权重都小于0.005？）";
CREATE TABLE recommend.tag_relation_user_preference_brand_simple AS select tag_id,user_proxy_key from (SELECT tag_id,user_proxy_key,user_tag_weight,count(*) over (partition by user_proxy_key) tag_sum, row_number () over (partition by user_proxy_key ORDER BY user_tag_weight DESC) rank FROM recommend.tag_relation_user_preference_brand) t where tag_sum>5 and rank<20 and user_tag_weight>0.005;

select "计算标签出现次数";
CREATE TABLE recommend.tag_relation_count_left AS SELECT tag_id,count(DISTINCT user_proxy_key) user_num,row_number () over (ORDER BY count(DISTINCT user_proxy_key) DESC) rank FROM recommend.tag_relation_user_preference_brand_simple GROUP BY tag_id;
create table recommend.tag_relation_count_right as select * from recommend.tag_relation_count_left;

select "计算共现矩阵";
CREATE TABLE recommend.tag_relation_user_preference_brand_simple_mirror as select tag_id,user_proxy_key from recommend.tag_relation_user_preference_brand_simple;
CREATE TABLE recommend.tag_relation_count_cross AS SELECT t.tag_id_1,t.tag_id_2,t.num FROM (SELECT t1.tag_id AS tag_id_1,t2.tag_id AS tag_id_2,count(DISTINCT t2.user_proxy_key) AS num FROM recommend.tag_relation_user_preference_brand_simple t1 CROSS JOIN recommend.tag_relation_user_preference_brand_simple_mirror t2 ON t1.user_proxy_key=t2.user_proxy_key WHERE t1.tag_id<> t2.tag_id GROUP BY t1.tag_id,t2.tag_id) t;
