...
spark-sql --driver-class-path /data/tmp/zhanshulin/jar/mysql-connector-java-commercial-5.1.40-bin.jar --name 'tag_relation_collaborative 品类品牌协同过滤' --driver-memory 8g --executor-memory 8G --executor-cores 4 --num-executors 3 --conf 'spark.default.parallelism=100' --master yarn --queue q_gmv --master yarn -e"
...
select '计算标签出现次数';
CREATE TABLE recommend.tag_relation_cate_count_left AS SELECT tag_id,count(DISTINCT user_proxy_key) user_num,row_number () over (ORDER BY count(DISTINCT user_proxy_key) DESC) rank FROM recommend.tag_relation_cate_user_preference_level_3_simple GROUP BY tag_id;
