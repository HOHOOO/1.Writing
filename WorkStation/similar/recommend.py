    ...
    def get(self):
        ...
        if redis_conn.exists(key) and shunt == "shunt_test":
            try:
                ...
                gen_log.info(
                    "device (%s) shunt to test env(url: %s)", device_id, url)
                request = urllib2.Request(url)
                response = urllib2.urlopen(request)
                shunt_result = response.read()
                gen_log.info("shunt device (%s) result len: %s",
                             device_id, len(shunt_result))
            except Exception as e:
                shunt_result = str(e)
            self.jsonify(shunt_result)
            return
        ha = HomeArticleB(redis_conn, ab_test_type,
                          device_id, smzdm_id, page, nums)
...
class FeedBackHandler(BaseHandler):
    ...
    def post(self):
        ...
        user_dislike_content_table = config.get(
            "mysql_table.user_dislike_content_table")
        user_dislike_content_full_table = config.get(
            "mysql_table.user_dislike_content_full_table")
        smzdm_category_table = config.get("mysql_table.smzdm_category")
        device_id = str(urllib.unquote(self.get_argument(
            "device_id", "", False)).encode("utf-8").replace(" ", "+"))  # 空格替换为+
