...
class YouHuiSimilarity(object):
    def __init__(self, current_article_frame, recommend_pools_frame, base_weights, extra_weights,
        ...
        self._use_rule()
        self.recommend_pools_num = self.recommend_pools.shape[0]
        gen_log.info("recommend_pools num after use_rule: %s" %
                     self.recommend_pools_num)
    ...
    def _use_rule(self):
        ...
        if self.current_article["level_2_id"].values[0] == str(SEX_PRODUCT_LEVEL_ID):
            self.recommend_pools = recommend_pools[recommend_pools["level_2_id"] == str(
                SEX_PRODUCT_LEVEL_ID)]
        else:
            self.recommend_pools = recommend_pools[recommend_pools["level_2_id"] != str(
                SEX_PRODUCT_LEVEL_ID)]
    ...
    def _calculate_similarity(self):
        ...
        for feature_col in self.feature_cols:
            ...
            try:
                ...
                recommend_pools_y = vect.fit_transform(
                    recommend_pools[feature_col])
                # gen_log.debug("vocabulary_: %s" % vect.get_feature_names())
                # transform出当前文章的矩阵
                current_article_X = vect.transform(
                    current_article[feature_col])
            except ValueError:
                gen_log.warn(
                    "empty vocabulary; perhaps the documents only contain stop words")
            # 计算出当前当前文章在该特征下与推荐池子中每篇文章的相似度
            simi_mat = cosine_similarity(
                current_article_X, recommend_pools_y, dense_output=False)
            ...
            # 将得到的特征相似度得分乘以对应的权重
            simi_mat_added_weight = simi_mat * \
                self.base_weights.get(feature_col)
            ...
            # 返回每个特征相似度得分时，采用原始结果
            simi_part_score[FEATURE_SCORE_NAME[feature_col]
                            ] = simi_mat.tolist()
        self.score_fields = simi_part_score.keys()  # 每个特征的相似度分数名称
        self.score_lists = simi_part_score.values()  # 每个特征的相似度分数列表
        self.sum_score = self._process_score_sum(
            simi_sum_score)  # 所有特征的相似度分数之和列表
    ...
    def _construct_result_data(self):
        ...
        ser = [ser.tolist()
               for col, ser in self.recommend_pools[self.frame_fields].iteritems()]
        result_data += ser
        # 每行结果为某条推荐结果
    ...
    def _construct_result(self):
        ...
        # 按照相似度总得分进行降序排列
        rec_data_list = sorted(
            result_data, key=lambda item: item[0], reverse=True)
        # 筛选大于相似度总分数大于指定值的结果
        rec_data_list = filter(
            lambda item: item[0] > self.threshold_simi_score, rec_data_list)[0: self.max]
        # 为每个字段添加名称
        rec_data_list = map(lambda items: dict(
            zip(result_fields, items)), rec_data_list)
        rec_result = self.rec_result
        rec_result["info"] = self.current_article[self.frame_fields].to_dict(
            "index").values()[0]
...
class YouHuiPreProcess(object):
    ...
    def process(self, frame):
        ...
        frame["level_1_id"] = frame["level_1_id"].fillna(
            value="").astype(str).str.replace("-1", "0")
        frame["level_2_id"] = frame["level_2_id"].fillna(
            value="").astype(str).str.replace("-1", "0")
        frame["level_3_id"] = frame["level_3_id"].fillna(
            value="").astype(str).str.replace("-1", "0")
        frame["level_4_id"] = frame["level_4_id"].fillna(
            value="").astype(str).str.replace("-1", "0")
