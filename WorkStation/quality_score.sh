...
SELECT id,score_alias AS score,last_status,CASE WHEN last_status=0 THEN 0 ELSE ((score_alias-last_status)/last_status+0.00001) END AS increase_rate,order_rank,score_timestamp,class FROM (SELECT id,score AS score_alias,score_timestamp,lead (score,1,0) over (PARTITION BY id ORDER BY score_timestamp DESC) AS last_status,rank () over (PARTITION BY id ORDER BY score_timestamp DESC) AS order_rank,class FROM (SELECT id,score,score_timestamp,class,order_rank FROM recommend.quality_data_score WHERE order_rank=1 UNION ALL SELECT id,score,score_timestamp,class,order_rank FROM recommend.quality_data_source) a ) b) c) t;
'
