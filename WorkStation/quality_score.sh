...
hive -d zscore='1.96' -d  para1='1/4' -d para2='1/4' -d para3='1/4' -d para4='1/4' -d store='0.8' -d timestamp=`date -d "now" +%s` -e '
use recommend;
...
CREATE TABLE recommend.quality_data_score_end AS INSERT OVERWRITE TABLE recommend.quality_data_score SELECT id,score,last_status,(increase_rate-min_increase_rate)/(max_increase_rate-min_increase_rate+0.00001) as increase_rate, order_rank,score_timestamp,class from (select id,score,last_status ,increase_rate, max(increase_rate) over (PARTITION BY class ) as max_increase_rate,min(increase_rate) over (PARTITION BY class ) as min_increase_rate,order_rank,score_timestamp from (SELECT id,score,last_status,CASE WHEN last_status=0 THEN 0 ELSE (score-last_status)/last_status END AS increase_rate,order_rank,score_timestamp,class FROM (SELECT id,score,score_timestamp,lag(score,1,0) over (PARTITION BY id ORDER BY score_timestamp ASC) AS last_status,rank () over (PARTITION BY id ORDER BY score_timestamp DESC) AS order_rank,class FROM (SELECT id,score,score_timestamp,class FROM recommend.quality_data_score WHERE order_rank=1 UNION ALL SELECT id,score,score_timestamp,class FROM recommend.quality_data_source) a) b) c where order_rank=1) t;
'
