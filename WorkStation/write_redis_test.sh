...
hive -S -e'set mapred.job.queue.name=q_gmv;
...
CREATE TABLE DW_CP_USER_PREFERENCE_${version}_REDIS_temp2 AS SELECT user_proxy_key ,regexp_extract(tag_str,"cate_(.*?)(#|$)", 1) AS cate,regexp_extract(tag_str,"brand_(.*?)(#|$)", 1) AS brand,regexp_extract(tag_str,"tag_(.*?)(#|$)", 1) AS tag from DW_CP_USER_PREFERENCE_${version}_REDIS_TEMP;
CREATE TABLE DW_CP_USER_PREFERENCE_${version}_REDIS AS SELECT user_proxy_key,CASE WHEN cate IS NULL OR cate="" THEN "-1:-1" ELSE cate END as cate,CASE WHEN brand IS NULL OR brand="" THEN "-1:-1" ELSE brand END as brand,CASE WHEN tag IS NULL OR tag="" THEN "-1:-1" ELSE tag END as tag from DW_CP_USER_PREFERENCE_${version}_REDIS_temp2;
