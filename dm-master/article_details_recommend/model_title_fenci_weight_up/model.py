...
class SimilaryModel(object):
    ...
    def __init__(self, dataframe, word_features_with_weight,
                 char_features_with_weight, src_article_num=None,
    ...
    def _generate_vector(self):
        ...
        tmp_features = copy.copy(self.word_features)
        for feature in tmp_features:
            try:
                feature_matrix = vect_word.fit_transform(data[feature])
        ...
        tmp_features = copy.copy(self.char_features)
        for feature in tmp_features:
            try:
                feature_matrix = vect_char.fit_transform(data[feature])
                feature_matrixs[feature] = feature_matrix
                # print "feature_matrixs[%s]:" % feature
                # print "feature_matrixs:\n %s" % feature_matrixs[feature].toarray()
                # print "feature: %s" % ", ".join(vect_word.get_feature_names())
        ...
        # print self.dataframe[["pro_id","level_4_id"]]
        # print vect_word.fit_transform(self.dataframe["pro_id"]).toarray()
    ...
    def calculate_similary(self):
        ...
        # feature = "title_fc"
        # print "feature_matrixs[%s]:" % feature
        # print "feature_matrixs:\n %s" % self.features_with_score[feature].toarray()
...
class ModelMain(object):
    def __init__(self, data, word_features_with_weight,
                 char_features_with_weight, src_article_num=None,
                 rec_article_num=3, title_fc_extra_weight=None,
                 rec_format="json", ndigits=2):
    ...
    def run_similary(self):
        model = SimilaryModel(self.data, self.word_features_with_weight,
                              self.char_features_with_weight, self.src_article_num,
                              self.rec_article_num, self.title_fc_extra_weight, self.ndigits)
        res_csr_matrix = model.calculate_similary()
        result = model.map_articles(res_csr_matrix, self.rec_format)
        return result

    def run_combine(self):
        simi = SimilaryModel(self.data, self.word_features_with_weight, self.char_features_with_weight)
...
if __name__ == "__main__":
    ...
    data = pp(d)
    # print data.head()
    # model = SimilaryModel(d, features_with_weight_1,
    #                       features_with_weight_2,
    #                        rec_article_num=3, title_fc_extra_weight=8,
    #                        ndigits=2)
    # res_csr_matrix = model.calculate_similary()
