...
from base.consts import AB_TEST_RANGE_A_NAME, AB_TEST_RANGE_B_NAME, AB_TEST_RANGE_C_NAME
from model.preprocessing import PreProcess
from tools.stop_words import SIGNAL_STOP_WORDS
from tools.vectorizer import MyCountVectorizer
...
class SimilaryModel(object):
    ...
    def map_articles(self, res_format="json"):
        ...
        for index in xrange(src_article_num):  # 每一行的数据表示一篇文章跟其他文章的相似度分数列表
            ...
            sum_score_row = np.around(self.score_csr_matrix.getrow(index).toarray()[0], self.ndigits)
            # print sum_score_row
...
class CombineModel(object):
    ...
    def map_articles(self, res_format="json"):
        ...
        for index in xrange(src_article_num):  # 每一行的数据表示一篇文章跟其他文章的相似度分数列表
            ...
            # model_version_list = [model_version] * len(self.dataframe)
            # 将推荐的文章id和相对应的相似度分数列表进行匹配
            rec_article_id_with_score = zip(self.dataframe["article_id"], sum_score_row, *single_score_row_list)

            # 按照总分来降序排序，总分相同，按照相似度分数降序排列，选出排名 Top N
            recs = sorted(rec_article_id_with_score, key=itemgetter(1, -1), reverse=True)[0: self.rec_article_num]
...
combine_weights = {"similary": 0.85, "heat": 0.05}
# model_version = AB_TEST_RANGE_A_NAME
# model_version = AB_TEST_RANGE_B_NAME
model_version = AB_TEST_RANGE_C_NAME
