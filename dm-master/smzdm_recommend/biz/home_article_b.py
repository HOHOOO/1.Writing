...
class HomeArticleB(object):
	...
	def _parse_prefer_info(self):
		...
		if self._device_id and (not self._prefer_flag):
			...
			try:
				...
				if prefer_json_data:
					v_json = HomeArticleB._parse_prefer_to_json(prefer_json_data)
					# cate
					...
					if cate_yh_accurate:
						for v in cate_yh_accurate:
							d = v.split(COLON)
							self._cate_prefer[YOUHUI_KEY][ACCURATE_KEY][u"%s" % d[0].strip()] = d[1].split(VERTICAL_LINE)
					
					if cate_yh_blur:
						for v in cate_yh_blur:
							d = v.split(COLON)
							self._cate_prefer[YOUHUI_KEY][BLUR_KEY][u"%s" % d[0].strip()] = d[1].split(VERTICAL_LINE)
					# 一级品类变异系数画像偏好
					# 数据格式为level_1_id: level1_weight#alpha
					if cate_yh_blur_para:
						for v in cate_yh_blur_para:
							d = v.split(COLON)
							self._cate_prefer[YOUHUI_KEY][PARA][u"%s" % d[0].strip()] = d[1].split(VERTICAL_LINE)
					if cate_yc_prefer:
						for v in cate_yc_prefer:
							d = v.split(COLON)
							self._cate_prefer[YUANCHUANG_KEY][u'%s' % d[0].strip()] = d[1].split(VERTICAL_LINE)
					...
					if tag_yh_prefer:
						for v in tag_yh_prefer:
							d = v.split(COLON)
							self._tag_prefer[YOUHUI_KEY][u'%s' % d[0].strip()] = d[1].split(VERTICAL_LINE)
					if tag_yc_prefer:
						for v in tag_yc_prefer:
							d = v.split(COLON)
							self._tag_prefer[YUANCHUANG_KEY][u'%s' % d[0].strip()] = d[1].split(VERTICAL_LINE)
					...
					if brand_yh_prefer:
						for v in brand_yh_prefer:
							d = v.split(COLON)
							self._cate_prefer[YOUHUI_KEY][u'%s' % d[0].strip()] = d[1].split(VERTICAL_LINE)
					if brand_yc_prefer:
	...
	def _get_query_dict(self, source, start_time, now, is_top=0, filter_flag=True):
		...
		if source not in [EDITOR_CHANNEL, YOUHUI_CHANNEL, YUANCHUANG_CHANNEL]:
			gen_log.warn("_get_query_dict source value error(source=%s).", str(source))
			raise gen.Return({})
		...
		if self._redis_conn.exists(self._yh_recommend_history_key):
			yh_recommend_history = self._redis_conn.lrange(self._yh_recommend_history_key, 0, -1)
		...
		if self._redis_conn.exists(self._yc_recommend_history_key):
			yc_recommend_history = self._redis_conn.lrange(self._yc_recommend_history_key, 0, -1)
		...
		if self._redis_conn.exists(self._history_article_list_key) and filter_flag:
			user_article_list = self._redis_conn.lrange(self._history_article_list_key, 0, -1)
	...
	def _get_recommend_article_from_es(self, source, limit):
		...
		if source not in [YOUHUI_CHANNEL, YUANCHUANG_CHANNEL]:
			gen_log.error("_get_recommend_article_from_es error source value(%s).", str(source))
			raise gen.Return([])
	...
	def rec_monitor_business(self):
		try:
			...
			gen_log.info("====rec_monitor_business start_time: %s===", start_time.strftime("%Y-%m-%d %H:%M:%S"))
			d = {
			...
			gen_log.info("====rec_monitor_business total_seconds: %s(%s)===", total_seconds.total_seconds(), d)
		except Exception as e:
	...
	def get_home_article_list(self):
		...
		if 1 == self._page:
			...
			if self._redis_conn.exists(delay_time_key):
				...
				pull_down_end_time = self._redis_conn.get(self._pull_down_last_time_key)
				if not pull_down_end_time:
					# expire_flag = True
					pull_down_end_time = (datetime.now() - timedelta(hours=24) - timedelta(minutes=2)).strftime("%Y-%m-%d %H:%M:%S")
				# 记录本次下拉的时间到缓存中
				now = (datetime.now() - timedelta(minutes=2)).strftime("%Y-%m-%d %H:%M:%S")
				self._redis_conn.set(self._pull_down_last_time_key, now)
				
				gen_log.info("page: %s, pull_down_end_time: %s", self._page, pull_down_end_time)
				...
				gen_log.info("page: %s, editor data len: %s", self._page, editor_data_len)
			...
			if editor_data_len >= RECOMMEND_B_SIZE:
				...
				if yh_recommend_len >= 1:
					...
					gen_log.info("candidate recommend device_id:%s:p3:yh:%s", self._device_id, str(yh_recommend[0]))
				if yc_recommend_len >= 1:
					editor_data.insert(4, yc_recommend[0])  # yc
					gen_log.info("candidate recommend device_id:%s:p4:yc:%s", self._device_id, str(yc_recommend[0]))
				if yh_recommend_len >= 2:
					editor_data.insert(7, yh_recommend[1])  # yh
					gen_log.info("candidate recommend device_id:%s:p7:yh:%s", self._device_id, str(yh_recommend[1]))
				if yc_recommend_len >= 2:
					editor_data.insert(8, yc_recommend[1])  # yc
					gen_log.info("candidate recommend device_id:%s:p8:yc:%s", self._device_id, str(yc_recommend[1]))
				if yh_recommend_len >= 3:
					editor_data.insert(9, yh_recommend[2])  # yh
					gen_log.info("candidate recommend device_id:%s:p9:yh:%s", self._device_id, str(yh_recommend[2]))
					
				gen_log.info("recommend article list size: %s", len(editor_data[0:20]))
				
				result_list = yield self._save_redis_and_fetch_top20(editor_data, len(editor_data))
				if len(result_list) < self._nums:
					data = self._redis_conn.lrange(self._history_key, 0, self._nums - 1)
				...
				redis_data = self._redis_conn.lrange(self._history_key, 0, self._nums - 1)
		...
		gen_log.info("page=%d, redis_data len: %d, filter result len: %d", self._page, result_list_len, len(result_list))
	...
	def get_top_article(self, start_time, end_time):
		...
		if article:
			...
			self._redis_conn.set(TOP_ARTICLE_B_KEY, json.dumps(one_row_dict), 60)
	...
	def _save_redis_and_fetch_top20(self, data, data_len):
		...
		try:
			...
			for i in range(0, data_len):
				index += 1
				pipe.lpush(self._history_key, json.dumps(data[data_len - i - 1]))
				try:
					...
					logger.error("_save_redis_and_fetch_top20 lpush _history_article_list_key error(%s)", str(e))
			...
			logger.error("_save_redis_and_fetch_top20 execute error(%s)", str(e))
		
		take_save_time = datetime.now()
		history_article_len = self._redis_conn.llen(self._history_article_list_key)
		if history_article_len > HISTORY_B_COUNT:
			self._redis_conn.ltrim(self._history_article_list_key, 0, HISTORY_B_COUNT - 1)
			self._redis_conn.expire(self._history_article_list_key, 3600 * 24 * 7)
	...
	def _calc_article_weight_or_sort(self, data, is_home=0, if_sort=False, threshold=0.0):
		...
		for d in data:
			...
			if score[u"total"] < threshold:
				gen_log.debug("article_id: %s, score(%s) < threshold (%s)", d["_source"]["article_id"], score, threshold)
				continue
	...
	def _calc_score(self, data):
		...
		if data:
						...
						if level3_ids_list:
							...
							for v in level3_ids_list:
								if v in self._cate_prefer[YOUHUI_KEY][ACCURATE_KEY].keys():
									...
									wv = float(self._cate_prefer[YOUHUI_KEY][ACCURATE_KEY][v][-1])
									
									# 判断此偏好是否小于阀值
									if wv <= self._w[W_THRESHOLD]:
										filter_flag = False
										score_detail_list.append(u"yh_level_accurate filter cid[{cid}],weight[{w}]".format(cid=v, w=wv))
										break
							...
							if no_prefer_flag and filter_flag:
								...
								score_detail_list.append(
									u"yh_level_accurate_default[{score}]=basic[{basic_w}]*random_weight[{w}]".format(
										score=default_value, basic_w=self._w[W_PORTRAIT_WIGHT][W_ACCURATE_CATE], w=random_w
									))
						...
						if level1_ids_list and filter_flag:
							...
							for v in level1_ids_list:
								if v in self._cate_prefer[YOUHUI_KEY][BLUR_KEY].keys():
									if no_prefer_flag:
										no_prefer_flag = False
									wv = float(self._cate_prefer[YOUHUI_KEY][BLUR_KEY][v][-1])
					...
					if brand_ids_list and filter_flag:
						...
						for v in brand_ids_list:
							if v in self._brand_prefer[YOUHUI_KEY].keys():
								...
								# 判断此偏好是否小于阀值
								wv = float(self._brand_prefer[YOUHUI_KEY][v][-1])
								if wv <= self._w[W_THRESHOLD]:
									filter_flag = False
									score_detail_list.append(
					...
					if level3_ids_list:
						...
						for v in level3_ids_list:
							if v in self._cate_prefer[YUANCHUANG_KEY].keys():
								...
								# 判断此偏好是否小于阀值
								wv = float(self._cate_prefer[YUANCHUANG_KEY][v][-1])
								if wv <= self._w[W_THRESHOLD]:
									filter_flag = False
									score_detail_list.append(
					...
					if tag_ids_list and filter_flag:
						...
						for v in tag_ids_list:
							if v in self._tag_prefer[YUANCHUANG_KEY].keys():
								...
								# 判断此偏好是否小于阀值
								wv = float(self._tag_prefer[YUANCHUANG_KEY][v][-1])
								if wv <= self._w[W_THRESHOLD]:
									filter_flag = False
									score_detail_list.append(
					...
					if brand_ids_list and filter_flag:
						...
						for v in brand_ids_list:
							if v in self._brand_prefer[YUANCHUANG_KEY].keys():
								...
								# 判断此偏好是否小于阀值
								wv = float(self._brand_prefer[YUANCHUANG_KEY][v][-1])
								if wv <= self._w[W_THRESHOLD]:
									filter_flag = False
									score_detail_list.append(
			...
			if sync_home > 0 and filter_flag:
				...
				score_detail_list.append(u"sync_home[{sync_home_w}]".format(sync_home_w=self._w[W_EDITOR_SYNC_TOTAL_WEIGHT]))
			...
			if filter_flag:
				...
				if sync_home_time >= self._half_hour_ago:
					...
					score_detail_list.append(u"half_hour[{hour_score}]".format(hour_score=self._w[W_TIME_WEIGHT][W_HALF_HOUR]))
				elif sync_home_time >= self._one_hour_ago:
					score_total += self._w[W_TIME_WEIGHT][W_HOUR_1]
					score_detail_list.append(u"one_hour[{hour_score}]".format(hour_score=self._w[W_TIME_WEIGHT][W_HOUR_1]))
				elif sync_home_time >= self._three_hour_ago:
					score_total += self._w[W_TIME_WEIGHT][W_HOUR_3]
					score_detail_list.append(u"three_hour[{hour_score}]".format(hour_score=self._w[W_TIME_WEIGHT][W_HOUR_3]))
				elif sync_home_time >= self._twelve_hour_ago:
					score_total += self._w[W_TIME_WEIGHT][W_HOUR_12]
					score_detail_list.append(u"twelve_hour[{hour_score}]".format(hour_score=self._w[W_TIME_WEIGHT][W_HOUR_12]))
				elif sync_home_time >= self._twenty_four_hour_ago:
					score_total += self._w[W_TIME_WEIGHT][W_HOUR_24]
					score_detail_list.append(u"twenty_hour[{hour_score}]".format(hour_score=self._w[W_TIME_WEIGHT][W_HOUR_24]))
	...
	def _calc_level_by_variation_coefficient(self, level1_ids_list, level3_ids_list, score_detail_list):
		gen_log.debug("level1_ids_list: %s, level3_ids_list: %s", str(level1_ids_list), str(level3_ids_list))
		...
		try:
			...
			for l1 in level1_ids_list:
				...
				gen_log.debug("_calc_level_by_variation_coefficient l1: %s, weight: %s", l1, weight)
				if weight:
					alpha, w1 = weight[0].split(WELL)
					alpha = float(alpha)
					w1 = float(w1)
					w1_score = (self._w[W_PORTRAIT_WIGHT][W_BLUR_CATE] - alpha) * w1
					...
					w3_score = 0.0
					compare_score = 0.0
					gen_log.debug("_calc_level_by_variation_coefficient alpha: %s, w1: %s", alpha, w1)
					for l3 in level3_ids_list:
						w3 = self._cate_prefer[YOUHUI_KEY][ACCURATE_KEY].get(l3, '')
						gen_log.debug("_calc_level_by_variation_coefficient l3: %s, w3: %s", l3, w3)
						if w3:
							w3 = float(w3[0])
							# 判断是否降权到过滤
							...
							w3_weight = (self._w[W_PORTRAIT_WIGHT][W_ACCURATE_CATE] + 2 * alpha) * w3
							
							gen_log.debug("_calc_level_by_variation_coefficient w3_weight: %s", w3_weight)
							
							if (w1_score + w3_weight) > compare_score:
								w3_score = w3_weight
								w3_score_detail = u"l3id={l3}, {score}=({basic} + 2*{alpha}) * {w3}]".format(l3=l3,
													basic=self._w[W_PORTRAIT_WIGHT][W_ACCURATE_CATE],
			...
			if filter_flag:
				...
				w1_score_detail = ''
				w3_score_detail = ''
			score_detail_list.append("%s\n%s" % (w1_score_detail, w3_score_detail))
		except Exception as e:
			gen_log.error("_calc_level_by_variation_coefficient error(%s)", str(e))
		gen_log.debug("_calc_level_by_variation_coefficient level_score_total: %s", level_score_total)
